  ...
  return (
    <div id="start-overlay" className="overlay open" hidden={state.menu !== "start"}>
      ...
      <main className="start-content">
        ...
        <form id="start-form" onSubmit={event => { event.preventDefault(); if (state.canContinue) ui.action("onContinue"); else ui.action("onStart", { seed: state.seed || util.randomSeedString(), mode: state.settings.mode }); }} autoComplete="off" noValidate>
          <button id="btn-start" disabled={!state.ready || !!state.error} className="btn primary" type="submit"><span>Start Exploring</span><span aria-hidden="true">↗</span></button>
        ...
        <ul id="hints-desktop" className="hints" hidden={state.touch}>
          ...
          <li><kbd>Esc</kbd> pause · <kbd>E</kbd> sit or interact nearby</li>
          <li>While seated, drag to look · <kbd>H</kbd> hide the controls</li>
        </ul>
        <ul id="hints-touch" className="hints" hidden={!state.touch}>
          <li>Left half: joystick appears under your thumb</li>
          <li>Right half: drag to look &middot; tap Sprint to move faster</li>
          <li>Hold Jump / up to surface &middot; hold Dive to swim down</li>
          <li>Tap Jump / up to jump on land</li>
          ...
          <li>Tap a nearby invitation to interact · drag to look while seated</li>
        </ul>
        </details>
      </main>
